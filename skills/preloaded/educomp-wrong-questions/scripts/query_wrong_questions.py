                ...
                for item in parsed:
                    ...
                    if endpoint_id and base_url and api_key:
                        endpoints.append(
                            {
                                "id": endpoint_id,
                                "name": str(item.get("name", endpoint_id)).strip() or endpoint_id,
                                "base_url": base_url,
                                "api_key": api_key,
                            }
                        )
...
def build_url(endpoint: dict, payload: dict) -> str:
    query = {}
    if payload.get("knowledge_point"):
        query["knowledgePoint"] = str(payload["knowledge_point"])
    if payload.get("mind_map_id"):
        query["mindMapId"] = str(payload["mind_map_id"])
    if payload.get("user_id"):
        query["userId"] = str(payload["user_id"])
    query["skip"] = str(payload.get("skip", 0))
    query["take"] = str(payload.get("take", 20))
    return endpoint["base_url"] + "/api/wrong-questions?" + urllib.parse.urlencode(query)
...
def main() -> int:
    try:
        ...
        endpoint = resolve_endpoint(payload, endpoints)
